...
export class Render {
  ...
  playlist(results) {
    ...
    results.forEach(item => {
      ...
      printString += `<div name="${item.data().videoLink}" id="${
        item.id
      }" class="playlist-box">
                <div class ="playlist--title"> ${item.data().videoName} </div>
                <div class="playlist--order"> ${item.data().order} <img src="${
        item.data().img
      }"></div>
                <div class="playlist--user"> ${item.data().user} </div>
                <div class="playlist--buttons">
                    <button class="btn btn-danger delete" name="${
                      item.id
                    }">delete song</button>
                    <button class="btn btn-success moveUp" name="${
                      item.id
                    }" value="${
        item.data().order
      }"><i class="fas fa-arrow-up"></i></button>
                    <button class="btn btn-success moveDown" name="${
                      item.id
                    }" value="${
        item.data().order
  ...
  roomList(rooms) {
    ...
    rooms.forEach(item => {
      printString += `<li>
                <div>
                    ${item.data().roomName} 
                    <button class="show-playlist" value="${
                      item.id
                    }">show-playlist</button>
                    <button class="show-main-show" value="${
                      item.id
                    }">go to main show</button>
                    <button class="show-invite" value="${
                      item.id
                    }">share link</button>
                    <button class="show-delete" value="${
                      item.id
                    }">Delete Room</button>
                </div>
                <div id="share-link-${item.id}" style="display:none;">
                    <input id="${
                      item.id
                    }-input" type="text" value="https://karaoke-team.web.app/?${
        item.id
      }"> 
                    <button name="${
                      item.id
                    }" class="copy-to-clipboard"> Copy Link </button>
                </div>
            </li>`;
    });
  ...
  updateCurrentSong() {
    this.currentSong = render.listObj[0];
    console.log("render.currentSong", render.currentSong);
    $(".current-song").append(`<li>${currentSong.videoName}</li>`)
    $(".current-song").append(`<li>${currentSong.user}</li>`)
  }
}
